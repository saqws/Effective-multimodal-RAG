...
TaskType = Literal['IT-IT', 'T-I', 'I-T', 'IT-T', 'T-IT', 'T-T']

def load_image_from_field(field):
    """
    ...
    if isinstance(field, dict) and 'bytes' in field:
        try:
            ...
            print(f"[Ошибка] Невозможно декодировать изображение: {e}")
            return None
    else:
        print("pic not found")
        return None
...
class MultimodalRetrievalDataset(Dataset):
    ...
    def _load_data(self):
        ...
        print("qrel loaded")
        self.query_to_docs = self.qrels.groupby('query-id')['corpus-id'].apply(list).to_dict()
        print("data loaded")
    ...
    def _build_indices(self):
        ...
        all_corpus_ids = set(self.corpus['id'])
    ...
    def _get_negatives(self, query_id: str, exclude_corpus_id: str) -> Tuple[List[str], List[Image.Image]]:
        ...
        while len(selected_indices) < num_negs:
            ...
            if random_idx != exclude_idx and random_idx not in selected_indices:
                ...
                if 'image' in row:
                    neg_images.append(load_image_from_field(row['image']))
    ...
    def __getitem__(self, idx):
        ...
        if 'image' in doc_row and pd.notnull(doc_row['image']):
            pos_image = load_image_from_field(doc_row['image'])
        
        query_image = None
        if 'image' in query_row and pd.notnull(query_row['image']):
            query_image = load_image_from_field(query_row['image'])
        
        # Получаем негативные примеры
        neg_texts, neg_images = self._get_negatives(query_id, corpus_id)
        
        # Формируем выходные данные в зависимости от типа задачи
        if self.task_type == 'IT-IT':
            # Input: Image+Text, Positive: Image+Text
            return {
                'task_type' : task_type,
        ...
        elif self.task_type == 'IT-T':
            # Input: Image+Text, Positive: Text
            return {
                'task_type' : task_type,
                'input_text': query_row['text'],
                'input_image': query_image,
        ...
        elif self.task_type == 'T-IT':
            # Input: Text, Positive: Image+Text
            return {
                'task_type' : task_type,
                'input_text': query_row['text'],
                'input_image': None,
...
# dataset_path = "UMRB-EncyclopediaVQA"
# task_type = "IT-IT"  
...
class CSVMultimodalRetrievalDataset(Dataset):
    ...
    def _load_data(self, dataset_percents):
        ...
        # self.data = pd.read_csv(os.path.join(self.dataset_path, "captions.csv"))
    ...
    def _get_negative_images(self, idx: int) -> list[Image.Image]:
        ...
        return [self.load_image_from_file(self.data.iloc[negative_index]["file_name"]) for negative_index in negative_indexes]
    
    def _get_positive_image(self, idx: int) -> Image.Image:
        # print("Getting pos images")
        return self.load_image_from_file(self.data.iloc[idx]["file_name"])
    ...
    def _get_negative_captions(self, idx: int) -> list[str]:
        ...
        while len(negative_captions) < self.num_negatives:
            ...
            list_captions = json.loads(self.data.iloc[idx]["caption"])
            negative_captions.append(list_captions[random.randint(0, len(list_captions))])
    ...
    def load_image_from_file(self, filename: str) ->Image.Image:
        ...
        if not os.path.exists(filepath):
            print(f"[Ошибка] Не найдено изображение {filepath}")
            return None
        try:
