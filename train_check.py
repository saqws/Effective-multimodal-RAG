...
class Trainer:
    def __init__(self):
        ...
        full_loader = loader.create_multimodal_dataloader(
            ...
            num_negatives=NUM_NEG, dataset_percents = 0.00025
)
    ...
    def _train_epoch(self, epoch):
        ...
        for step, b in enumerate(self.tr_loader, 1):
            q = self._embed(b["query"]); p = self._embed(b["pos"])
    ...
    def _build_val_index(self):
        self.model.eval()
        Q_list,D_list,gold=[],[],[]; cursor=0
        for b in self.val_loader:
            q   = self._embed(b["query"]).cpu()
            pos = self._embed(b["pos"]).cpu()
            neg = self._embed(b["neg"]).cpu()
            B = q.size(0)
            D_list.extend(pos); gold.extend(range(cursor,cursor+B)); cursor+=B
            D_list.extend(neg); cursor+=neg.size(0)
            Q_list.append(q)

            torch.cuda.empty_cache()

        Q = F.normalize(torch.cat(Q_list),1).half()
        D = F.normalize(torch.stack(D_list),1).half()
        return Q,D,torch.tensor(gold)
    
    @torch.no_grad()
    def _metrics(self,Q,D,gold):
        N=Q.size(0); D_gpu=D.T.to(DEVICE)
        hits={k:[] for k in TOP_KS}; ranks=[]; probs=[]
        for s in range(0,N,VAL_CHUNK):
            e=min(s+VAL_CHUNK,N)
            sims = (Q[s:e].to(DEVICE) @ D_gpu)
            soft = F.softmax(sims/TEMP,1)
            probs.append(soft[torch.arange(e-s), gold[s:e].to(DEVICE)].cpu())
            order = sims.argsort(1,descending=True).cpu()
            ranks.append(((order==gold[s:e].unsqueeze(1)).nonzero()[:,1])+1)
            for k in TOP_KS:
                hits[k].append((order[:,:k]==gold[s:e].unsqueeze(1)).any(1).float())
    ...
    def fit(self):
        for ep in range(1,EPOCHS+1):
            ...
            if ep % VAL_EVERY_EPOCHS == 0:
                    Q, D, gold = self._build_val_index()
                    val = self._metrics(Q, D, gold)
                    log.update({f"val_{k}": v for k, v in val.items()})
                    print(f"VAL EP{ep}:", {k: f"{v:.4f}" for k, v in val.items()})
                    del Q, D, gold; torch.cuda.empty_cache()
